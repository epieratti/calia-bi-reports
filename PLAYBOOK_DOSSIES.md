...
| PDF (layout + cores, pós-gate) | `python3 tools/dossier_export_pdf.py --html <…> --out <…>.pdf` | Playwright + Chromium; senha via `--password` ou `DOSSIER_PDF_PASSWORD`. `make dossie-pdf HTML=… OUT=…`. Requisitos: `pip install -r tools/requirements-pdf.txt` e `playwright install chromium`. |
