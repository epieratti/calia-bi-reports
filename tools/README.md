...
- **PDF (UI preservada):** `dossier_export_pdf.py` — Playwright imprime o HTML **após** o gate (servidor HTTP local na **raiz do repo**, para funcionar `../assets` e Chart.js). Após `emulate_media(print)`, o script dispara `resize` nos gráficos Chart.js para o PDF não sobrepor texto quando `@media print` muda alturas de containers. `pip install -r tools/requirements-pdf.txt && playwright install chromium`. Senha: `--password` ou `DOSSIER_PDF_PASSWORD`; **`--skip-gate`** gera sem senha (uso interno). Dossiês com vários gráficos: `--post-unlock-wait 4`. Ver `docs/GOVERNANCA_ENTREGA.md`.
