...
- **Build canônico:** `loterias2026/scripts/build_dossier_completo.py` (`--project-root` para cada lote)
- **Sincronização publicado vs fonte:** `check_dossier_publish_sync.py` — `make check-dossier-publish-sync` ou workflow `dossier-build-sync.yml` (usa `DOSSIER_BUILD_DATE` do HTML publicado ao comparar)
