...
**Execução real (31/03/2026):** `pip install -r engine/requirements/osint.txt` (espelho em `projects/caixa/loterias/always-on-20260401/research/osint_runs/requirements-osint.txt`) — **Instaloader** → **403** no GraphQL do Instagram; **Sherlock** + **yt-dlp** → resultados em `projects/caixa/loterias/always-on-20260401/research/osint_runs/20260331/` e resumo em `FONTES_BRAND_SAFETY_LOTERIAS2026.md` (seção **Rodada OSINT**).

**Dossiê HTML (entrega ao cliente):** gerado por `engine/cli/build_dossier.py` a partir de `data/dossier_loterias2026.md` + `data/dossier_loterias2026_panels.yaml`. O texto **na página** não deve citar arquivos internos — só linguagem de briefing. Evidências com URLs permanecem em **FONTES** (uso interno).
