...
5. Na raiz do repo: `python3 engine/qa/validate_source.py loterias2026/data/dossier_SEU_SLUG.md` (ou `make validate-dossier-squad-13` para o arquivo de referência desta pasta; o alvo `validate-dossier-13` continua como alias). Ver regras de Markdown em **`PLAYBOOK_DOSSIES.md`** → Toolbox.
...
- **`data/dossier_loterias2026_panels.yaml`** — só as **tabelas de métricas** (Instagram, TikTok, YouTube, X) e notas de cobertura. **Instagram e YouTube:** **Social Blade** (manual no navegador → `rows`). **TikTok:** **Upfluence** — você exporta/envia e organizamos nas `rows`. **X:** **na própria plataforma X** (manual) — registar **seguidores** e se a conta está **ativa** (ainda posta / uso recente) ou **parada há muito tempo**, mais uma linha de **teor recente** no formato da tabela (ver exemplos em `data/dossier_loterias2026_panels.yaml`).
